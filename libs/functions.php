...
function getCoursesByFilters($categoryId,$keyword,$page){
    ...
    $query = "";
    if(!empty($categoryId)){
        $query = "FROM `kurs_kategori` kc inner join kurslar k on kc.kategori_id = k.id WHERE kc.kurs_id = $categoryId and onay = 1";
    }
    else{
    ...
    $count_data = mysqli_query($baglanti,$query);
    $sonuc = mysqli_query($baglanti,$query);
...
function getCatergoriesById(int $courseId){
    include "ayar.php";
...
function getCourseById(int $id){
    ...
    function getCoursesByCategoryId(int $id){
        include "ayar.php";
        $query = "SELECT * FROM `kurs_kategori` kc inner join kurslar k on kc.kategori_id = k.id WHERE kc.kurs_id = $id";
        $sonuc = mysqli_query($baglanti,$query);
            mysqli_close($baglanti);
            return $sonuc;
    }
...
function editCourse(int $id, string $baslik,string $aciklama, string $altBaslik, string $resim, int $onay,int $anasayfa) {
    include "ayar.php"; 
...
function createCategory($kategori){
    include "ayar.php";
